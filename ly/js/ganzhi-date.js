/* ==================================================================
  ly/js/ganzhi-date.js —— 六十甲子与空亡、起卦日柱推算、年月时柱（节气）
...
// ---- 起卦日的日柱：不需要用户手动选，排盘时直接现算"起卦当天"落在六十甲子的第几个
// （对应桌面版 ganzhi.py 的算法，用同一批日期校准过）----
...
//    去查它的下标；跟getTodayJiaziIndex()用的是同一份JIAZI60数据源，不会出现两边表对不上的情况。
...
//    只有这一份代码在算，"起卦日柱""干支日应期换算"两处用的是同一个函数，
...
/* ==================================================================
   年月时柱（干支）：跟日柱一起凑成"年月日时"四柱。
   日柱和这里的年柱、月柱、时柱都按"起卦这一刻"现算，各算各的口径但同一个时刻，
   用户不用填任何一样。
   六爻真正要用的只有日柱（起六神、算空亡），年月日时四柱是给用户/AI看的一份
