/* ==================================================================
  ...
  本文件同时是"结构化六爻"字段契约的出处（buildPlateRows 生产它、structLineToDiagram 换算它）。
  （卦象爻画图那个函数在 diagram.js 里，本文件只是调用它）
...
/* ---------------- renderPlate 的分工小函数 ----------------
   ...
   （卦象爻画图那一段在 diagram.js 里，renderPlate 只是把它插在表格和卦名信息行之间）
   ---------------- */
...
// ---- "结构化六爻"字段契约（全站唯一出处；要加字段、改字段名，从这里开始改）----
// 一行一爻、初爻→上爻，字段如下：
//   爻位 / 六亲 / 六神 / 纳甲 / 五行 —— 展示用文本
//   状态 —— 只有四种写法：少阳、少阴、老阳→变阴、老阴→变阳
//   是否动爻 / 是否世爻 / 是否应爻 / 是否空亡 —— 布尔
//   变纳甲 / 变五行 / 变六亲 —— 只有动爻有值，其余为空串
//   伏神六亲 / 伏神纳甲 / 伏神五行 —— 该爻没有伏神时为空串
// 生产：本文件 buildPlateRows()；ai-core.js 存历史快照时原样转存这一份。
// 消费：ai-core.js（提示词、历史记录卡片）、renderPlateFromCastData()（刷新后恢复会话）。
// structLineToDiagram() 把这份契约换算成 diagram.js 要的几何形状
// （{yang, moving, isWorld, isResponse}），所以画图模块不需要认识上面这些字段名——
// 以后加字段或改字段名，只要动这里和那两个生产/消费方。
